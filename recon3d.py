...
def reconstruct3D(transform_candidates, calibrated_1, calibrated_2):
  ...
  for candidate in transform_candidates:
    ...
    for i in range(calibrated_1.shape[0]):
      QminusRPI = np.hstack((calibrated_2[i].reshape(-1,1), - R@calibrated_1[i].reshape(-1,1)))
      lambdas[:,i] = np.linalg.pinv(QminusRPI)@(T)
    # print(np.shape(QminusRPI))
    # x = np.linalg.pinv(QminusRPI)
    # print(x)
    # print(np.shape(x))
